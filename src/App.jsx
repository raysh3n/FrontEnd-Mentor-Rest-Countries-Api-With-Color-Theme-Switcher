...
export default function App() {
 ...
 const toggleTheme= ()=>{       //  const toggleTheme = function () {
  ...
  return <ThemeContext.Provider value={{ darkTheme, toggleTheme }}>
   ...
   <div className="bg-[hsl(0,0%,99%)] dark:bg-[hsl(207,26%,17%)] min-h-screen transition-color duration-100"> 
    ...
    <Routes>
      <Route path="/" element={
        <div>
        <Filters selected={selected} setSelected={setSelected} search={search} setSearch={setSearch} /> <CardList selected={selected} setSelected={setSelected} search={search} setSearch={setSearch} />
        </div>
      }>
      </Route>
      <Route path="/:countryName" element={<CountryDetail></CountryDetail>}></Route>
    </Routes>
</div>
